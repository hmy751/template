...
## 11. 컴포넌트 관리 문제 (2025-01-15)

### 문제 제기

> "컴포넌트를 보니 이게 맞나 싶기도 한데... 여러 파일들도 있고 컴포넌트라 안 맞기도 한 것 같아"

기존 정책(기술별 cheatsheet)이 컴포넌트에 맞지 않음.

### 분석: 라이브러리 vs 컴포넌트

| | 라이브러리 (react-query) | 컴포넌트 (Button 등) |
|---|---|---|
| **성격** | API 사용법 암기 | 코드 복붙 + 커스터마이징 |
| **질문** | "useQuery 옵션 뭐였지?" | "폼 전체 어떻게 만들지?" |
| **문서 필요성** | 높음 (외워야 함) | 낮음 (코드가 문서) |

### 스토리북의 한계

> "Form 같은 경우 빨리 차용하고 싶을 수 있잖아? 스토리북에만 있으면 활용도가 떨어질 것 같은데"

- 스토리북: "어떻게 생겼나" (개별 컴포넌트)
- 부족한 것: "어떻게 조합하나" (실전 패턴)

### 새로운 문제: 기술 중심 분류의 한계

> "상황 조합에 react-query 패턴이 있어. 그럼 이건 상황 조합 문서에도 있고 react-query에도 정리해야 하는 건가?"

예: 무한 스크롤
- `components/`: InfiniteScrollContainer 조합
...
과제할 때 질문은:
- ❌ "react-query 문법 뭐였지?" (기술 중심)
- ✅ "무한 스크롤 어떻게 만들지?" (상황 중심)

**상황 중심으로 찾는데, 기술 중심으로 분류해놓으면 안 맞음**

### 결정: 두 가지로 관리

> "나눠서 두 개로 관리하는 건 어때? 기술 중심 디렉토리 관리도 하고 상황 중심도 하고?"
...
- 질문: "무한 스크롤 어떻게 만들지?"
- 용도: 과제할 때 바로 복붙
- 구조: 주제별 단일 파일 (data-fetching.md, state.md, ui.md)

### 중복에 대해

> "물론 중복되는 부분도 있겠지만 어쩔 수 없이 그래도 관리를 다르게 나눠서 하는 거지"

- libs/와 recipes/에 같은 코드 있을 수 있음
- **관점이 다르므로 괜찮음**
  - libs: "이 기술 이해하기"
  - recipes: "이 상황 해결하기"
- 각각 독립적으로 관리
...
| 확인하고 싶은 것 | 어디서 |
|------------------|--------|
| 개별 컴포넌트 모양/props | 스토리북 |
| 컴포넌트 조합 패턴 | recipes/ui.md |
...
6. **기술 중심 + 상황 중심**: 둘 다 필요함. 중복 있어도 관점이 다르면 OK
